...
async (req, res, next) => {
    ...
    if (user) {
        const result = await usage_tracking_service_1.usageTrackingService.canManageBusiness(user.authId.toString());
        if (!result.allowed) {
            throw new ApiError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, result.reason);
        }
    }
    // flag the flow
...
router.delete('/facebook/disconnect', (0, auth_1.default)(user_1.USER_ROLES.ADMIN, user_1.USER_ROLES.USER), custom_auth_controller_1.CustomAuthController.disconnectFacebook);
...
router.delete('/instagram/disconnect', (0, auth_1.default)(user_1.USER_ROLES.ADMIN, user_1.USER_ROLES.USER), custom_auth_controller_1.CustomAuthController.disconnectInstagram);
// for tiktok
