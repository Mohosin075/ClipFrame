...
const disconnectFacebook = async (user) => {
    const isUserExist = await user_model_1.User.findById(user.authId);
    if (!isUserExist) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, 'User not found');
    }
    const result = await socialintegration_model_1.Socialintegration.findOneAndDelete({
        user: user.authId,
        platform: 'facebook',
    });
    if (!result) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, 'Facebook account is not connected');
    }
    return { message: 'Facebook disconnected successfully' };
};
const disconnectInstagram = async (user) => {
    const isUserExist = await user_model_1.User.findById(user.authId);
    if (!isUserExist) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, 'User not found');
    }
    ...
    if (!result) {
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, 'Instagram account is not connected');
    }
