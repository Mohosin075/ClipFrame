...
class SubscriptionService {
    ...
    async getAllPlans() {
        try {
            const plans = await subscription_plan_model_1.SubscriptionPlan.find({}).sort({
                priority: 1,
                price: 1,
            });
            return plans;
    ...
    async createSubscription(userId, request) {
        try {
            ...
            if (existingSubscription) {
                // If user has a free subscription, cancel it to allow for a paid one
                const existingPlan = existingSubscription.planId;
                if (existingPlan && existingPlan.price === 0) {
                    await subscription_model_1.Subscription.findByIdAndUpdate(existingSubscription._id, {
                        status: 'canceled',
                        endedAt: new Date(),
                    });
                }
    ...
    async handleFreeSubscriptionCreate(userId) {
        try {
            ...
            let freePlan = await subscription_plan_model_1.SubscriptionPlan.findOne({
            ...
            if (!freePlan) {
                ...
                freePlan = await subscription_plan_model_1.SubscriptionPlan.create({
                    name: 'Free Plan',
            ...
            if (!freePlan) {
                throw new ApiError_1.default(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, 'No active free subscription plan found in the system and failed to create one.');
            }
            const now = new Date();
            ...
            const subscription = new subscription_model_1.Subscription({
                userId: new mongoose_1.Types.ObjectId(userId),
                planId: freePlan._id,
                stripeCustomerId: 'free_plan', // Dummy for free plan
                stripeSubscriptionId: `free_${userId}_${now.getTime()}`,
                stripePriceId: freePlan.stripePriceId || 'free_price',
                status: 'active',
                currentPeriodStart,
                currentPeriodEnd,
                hasUsedTrial: false,
            ...
            await user_model_1.User.findByIdAndUpdate(userId, {
                ...
                subscriptionTier: freePlan.tier || 'free',
                subscriptionExpiresAt: currentPeriodEnd,
            });
            return subscription;
        }
            ...
            if (error instanceof ApiError_1.default)
                throw error;
            throw new ApiError_1.default(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to automatically create free subscription plan for you. Please try again or contact support.');
        }
    }
    ...
    async updateSubscription(userId, subscriptionId, request) {
        try {
            ...
            if (request.planId) {
                ...
                await user_model_1.User.findByIdAndUpdate(userId, {
                    subscriptionTier: newPlan.tier || 'free',
                });
