...
class WebhookService {
    ...
    async processWebhookEvent(event) {
        try {
            ...
            if (existingSubscription || existingPlan) {
    ...
    async handleSubscriptionUpdated(stripeSubscription, eventId) {
        try {
            ...
            if (newTier) {
                ...
                // Ensure tier is updated if status changed
                const currentPlan = await subscription_plan_model_1.SubscriptionPlan.findById(subscription.planId);
                if (currentPlan) {
                    userUpdate.subscriptionTier = currentPlan.tier;
                }
            }
        ...
        try {
            ...
            if (session.subscription) {
                ...
                await subscription_model_1.Subscription.findOneAndUpdate({ stripeSubscriptionId: stripeSubscription.id }, {
                    status: stripeSubscription.status,
                    lastWebhookEventId: eventId,
                    price: (plan === null || plan === void 0 ? void 0 : plan.price) || 0,
                    usage: {
                        reelsUsed: 0,
                        postsUsed: 0,
                        storiesUsed: 0,
                ...
                await user_model_1.User.findByIdAndUpdate(userId, {
                    ...
                    subscriptionTier: (plan === null || plan === void 0 ? void 0 : plan.tier) || 'free', // Ensure tier is updated on checkout
                });
            }
