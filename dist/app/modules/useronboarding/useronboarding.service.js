...
const createUseronboarding = async (user, payload) => {
    ...
    try {
        const existing = await useronboarding_model_1.Useronboarding.findOne({ userId });
        ...
        if (!existing) {
            const data = {
                ...
                preferredLanguages: preferredLanguages || [useronboarding_interface_1.ContentLanguage.EN],
            };
            return await useronboarding_model_1.Useronboarding.create(data);
        }
        ...
        const updated = await useronboarding_model_1.Useronboarding.findOneAndUpdate({ userId }, updateOps, {
            new: true,
            runValidators: true,
        }).lean();
        if (!updated) {
...
const getMyOnboarding = async (user) => {
    const result = await useronboarding_model_1.Useronboarding.findOne({ userId: user.authId }).populate('userId');
    return result;
};
const updateMyOnboarding = async (user, payload) => {
