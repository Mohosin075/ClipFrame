...
node_cron_1.default.schedule('*/5 * * * *', async () => {
    ...
    try {
        ...
        for (const item of contents) {
            ...
            const allPublished = platformStatus.size > 0 &&
                Array.from(platformStatus.values()).every(status => status === 'published');
            if (allPublished) {
                item.status = content_constants_1.CONTENT_STATUS.PUBLISHED;
                await item.save();
